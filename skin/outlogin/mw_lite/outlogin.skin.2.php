...
$sql = " select (TO_DAYS('{$g5['time_ymdhis']}') - TO_DAYS('{$member['mb_datetime']}') + 1) as days ";
$row = sql_fetch($sql);
...
<div class="box-inside">
    ...
    <div class="login-title"><a href="<?php echo G5_ADMIN_URL?>/"><strong><?php echo $nick?></strong> 님 어서오세요.</a></div>
    <?php } else { ?>
    <div class="login-title"><strong><?php echo $nick?></strong> 님 어서오세요.</div>
    <?php } ?>
    <div class="login-memo"><a href="<?php echo G5_BBS_URL?>/memo.php" class="win_memo"><strong>쪽지</strong>: <span class="login-memo-count"><?php echo $memo_not_read?></span></a></div>
    <div class="login-point"><a href="<?php echo G5_BBS_URL?>/point.php" class="win_point"><strong>포인트</strong>: <span class="login-point-number"><?php echo $point?></span> 점</a></div>
    <div class="login-level"><strong>권한</strong>: <?php echo $member['mb_level']?></div>
    <div class="login-days"><strong>가입일</strong>: <?php echo $mb_reg_after?> 일</div>
    <div class="login-membership">
        <a href="<?php echo G5_BBS_URL?>/scrap.php" class="win_scrap">스크랩</a> <span>|</span>
